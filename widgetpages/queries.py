...
(
    ...
    from
    ( 
		select distinct s.cust_id, s.budgets_id, s.PlanTYear from [dbo].[org_Contract_{{ org_id }}] s
        left join db_lpu l on s.cust_id = l.cust_id
        left join db_TradeNR t on s.{{ market_type_prefix }}TradeNx = t.id
        where  PlanTYear is not null
        {% if no_target %} 
...
q_lparts_count = """
{% autoescape off %}
select COUNT_BIG(DISTINCT s.cust_id) from [dbo].[org_Contract_{{org_id}}] s 
            left join db_lpu l on s.cust_id = l.cust_id
            left join db_TradeNR t on s.{{ market_type_prefix }}TradeNx = t.id
            where PlanTYear is not null 
            {% if no_target %} 
...
select pvt.cust_id as id, pvt.{{ market_type_prefix }}{{ product_type }} as tradeNx, grouping(pvt.{{ market_type_prefix }}{{ product_type }}) as gr
    ...
    (
        ...
        from [dbo].[org_Contract_{{org_id}}] s with (nolock)
        left join db_lpu l on s.cust_id = l.cust_id
...
and exists ( select 1 from org_Contract_{{ org_id }} s
...
where exists ( select 1 from org_Contract_{{ org_id }} s
               {% if no_target %}left join db_lpu l on s.cust_id=l.cust_id{% endif %}
...
where exists
        ( select top 1 1 from org_Contract_{{ org_id }} s
          {% if no_target %}left join db_lpu l on s.cust_id=l.cust_id{% endif %}
